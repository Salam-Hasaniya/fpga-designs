...
void Keyboard_Chasing_LEDs(Ps2Core *ps2_p, GpoCore *led_p, SsegCore *sseg_p) {
    uint32_t led_state = 0x0001;
    int direction = 0;
    int paused = 0;
    int setting_speed = 0;
    ...
    while (1) {
        if (ps2_p->get_kb_ch((char*)&ch)) {
            if (ch == 'p' || ch == 'P') {
                paused = !paused;
            }
            else if (ch == 0xf0) {  // F1 key release triggers speed input
                setting_speed = 1;
                digit_idx = 0;
                paused = 1;  // Pause during speed entry
                uart.disp("Entering speed input...\n\r");
            }
            else if (setting_speed) {
                if (ch >= '0' && ch <= '9') {
                    uart.disp("Digit entered: ");
                    uart.disp(ch);
                    uart.disp("\n\r");

                    digits[digit_idx++] = ch;
                    if (digit_idx == 3) {
                        speed = (digits[0] - '0') * 100 + (digits[1] - '0') * 10 + (digits[2] - '0');
                        setting_speed = 0;
                        paused = 0;

                        uart.disp("Speed set to: ");
                        uart.disp(speed);
                        uart.disp(" ms\n\r");
                    }
                } else {
                    // Invalid key during speed entry — just ignore
                    uart.disp("Ignored non-digit key during speed input.\n\r");
                }
            }
        }
        ...
        // Display
        if (paused) {
            sseg_p->write_1ptn(0x8C, 7);  // 'P'
            dp_mask |= (1 << 7);
        } else {
            sseg_p->write_1ptn(0xff, 7);  // blank
            dp_mask &= ~(1 << 7);
        }

        sseg_p->write_1ptn(0x92, 5); // 'S'
        sseg_p->write_1ptn(0x8C, 4); // 'P'
        sseg_p->write_1ptn(0xB7, 3); // ':' (G + D)

        sseg_p->write_1ptn(sseg_p->h2s(speed % 10), 0);
        sseg_p->write_1ptn(sseg_p->h2s((speed / 10) % 10), 1);
        sseg_p->write_1ptn(sseg_p->h2s((speed / 100) % 10), 2);

        sseg_p->set_dp(dp_mask);
...
void ps2_f1_test(Ps2Core *ps2_p) {
    unsigned char ch;
    static int expecting_second = 0;
    static unsigned char first_byte = 0;
    unsigned long last = now_ms();

    uart.disp("\n\r[PS/2 F1 Key Test Started] Press F1 or any key...\n\r");

    while (now_ms() - last < 10000) {  // run for 10 seconds
        if (ps2_p->get_kb_ch((char*)&ch)) {
            if (!expecting_second && ch == 0xE0) {
                // Extended key prefix
                first_byte = ch;
                expecting_second = 1;
                uart.disp("Got extended prefix: 0xE0\n\r");
            }
            else if (expecting_second) {
                uart.disp("Extended key sequence: ");
                uart.disp("0x");
                uart.disp(first_byte, 16);
                uart.disp(" 0x");
                uart.disp(ch, 16);
                uart.disp("\n\r");
                expecting_second = 0;
                ...
                uart.disp("Single byte scan code: 0x");
                uart.disp(ch, 16);
                uart.disp(" (int: ");
                uart.disp((int)ch);
                uart.disp(")\n\r");
            ...
            last = now_ms();  // reset timeout
        }
    }

    uart.disp("[Test finished]\n\r");
}
   ...
   while (1) {
      ...
      //adsr_check(&adsr, &led, &sw);
	   ...
	   //CUSTOM
	   	   ...
	   	   Keyboard_Chasing_LEDs(&ps2, &led, &sseg);
	   	   //ps2_f1_test(&ps2);
