...
void BlinkerCore::set_blink_rate(int led, uint16_t rate) {
   if (led < 0 || led > 3) return; // check if Valid LED range
   //io_write(base_addr, led, (uint32_t)(rate & 0xFFFF)); // Write 16-bit value to blinker register
   io_write(base_addr, led << 2, (uint32_t)(rate & 0xFFFF));
}
