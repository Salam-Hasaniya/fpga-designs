...
void light_sensor_check(SpiCore *spi_p, GpoCore *led_p) {
   int msb, lsb, light_val, num_leds;
   uint16_t raw, led_mask;

   spi_p->set_freq(1000000);  //1 MHz (datasheet recommends 1–4 MHz)
   spi_p->set_mode(0, 0);     //Mode 0 (data is clock out on falling edge, clocked in on rising edge)

   while (1) {
      spi_p->assert_ss(0);			 //turn on
      msb = spi_p->transfer(0x00);  //First byte (15:8)
      lsb = spi_p->transfer(0x00);  //Second byte (7:0) WE WANT 11:4
      spi_p->deassert_ss(0);		 //turn off

      //combine first and second byte
      raw = (msb << 8) | lsb;
      //extract [11:4] --> [7:0]
      light_val = (raw >> 4) & 0xFF;


      num_leds = map(light_val, 0, 255, 0, 17);
      led_mask = (1 << num_leds) - 1;

      led_p->write(led_mask);

      uart.disp("Ambient Light: ");
      uart.disp(light_val);
      uart.disp(" | LEDs Lit: ");
      uart.disp(num_leds);
      uart.disp("\n\r");
...
SpiCore light_spi(get_slot_addr(BRIDGE_BASE, S4_SPI_LS));  // slot 4 - light sensor SPI
...
int main() {
   ...
   while (1) {
      ...
      //adsr_check(&adsr, &led, &sw);
	   light_sensor_check(&light_spi, &led);  // <- new test app for light sensor
